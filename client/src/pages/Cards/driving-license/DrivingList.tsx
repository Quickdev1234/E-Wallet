...
const DrivinglicenseList = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      return await _axios.get(
        `/user/card/drivinglicense/allcards?userId=${userId}&page=${pagination.page}&limit=${pagination.limit}`
      );
    },
  });
  ...
  const { mutate: deleteMutate, isPending } = useMutation({
    mutationFn: () => {
      return _axios.delete(`/user/card/drivinglicense/delete?cardId=${cardId}`);
    },
    onSuccess: (data: any) => {
    ...
    <main className=' p-3 md:p-6 h-full font-grotesk bg-gradient-to-b from-gray-100 to-gray-200 min-h-[calc(100vh-4rem)] sm:min-h-[calc(100vh-5rem)]'>
      <div className=''>
        <div className='flex justify-between flex-wrap  items-center'>
          ...
          <button
            onClick={() => navigate("/upload-drivinglicense-card")}
            ...
            <div
              ...
              <div className='space-y-2'>
                <p className='text-sm text-gray-600 flex items-center'>
                  ...
                  Card Number: {card.licenseNumber}
                </p>
                <p className='text-sm text-gray-600 flex items-center'>
                  <span className='font-medium mr-2'>
                    <Icon
              ...
              <div className='mt-4 flex justify-end gap-5 border-t pt-3 text-gray-500'>
                <Icon
                  ...
                  onClick={() => {
                    navigate(`/upload-drivinglicense-card/${card._id}`);
                    useDrivingLicenseStore.setState({
