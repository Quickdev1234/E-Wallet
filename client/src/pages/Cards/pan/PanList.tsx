...
const PanList = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["Pan", pagination.page, pagination.limit],
    queryFn: async () => {
      return await _axios.get(
        `/user/card/pancard/allcards?userId=${userId}&page=${pagination.page}&limit=${pagination.limit}`
      );
    },
  });
  ...
  const { mutate: deleteMutate, isPending } = useMutation({
    mutationFn: () => {
      return _axios.delete(`/user/card/pancard/delete?cardId=${cardId}`);
    },
    onSuccess: (data: any) => {
    ...
    <main className=' p-3 md:p-6 h-full font-grotesk bg-gradient-to-b from-gray-100 to-gray-200 min-h-[calc(100vh-4rem)] sm:min-h-[calc(100vh-5rem)]'>
      <div className=''>
        <div className='flex justify-between flex-wrap  items-center'>
          ...
          <button
            onClick={() => navigate("/upload-pan-card")}
            ...
            <div
              ...
              <div className='mt-4 flex justify-end gap-5 border-t pt-3 text-gray-500'>
                <Icon
                  ...
                  onClick={() => {
                    navigate(`/upload-pan-card/${card._id}`);
                    usePanCardStore.setState({
                      currentPanCard: {
                        cardId: card._id,
